-- ============================================================================
-- Fix: settled competitive matches weren't advancing the placement counter.
-- 2026-07-13. Run once in the Supabase SQL editor. Idempotent.
--
-- The rating-engine-v2 _settle_rating() bumped competitive_matches but not
-- profiles.placement_played — yet the whole placement/ranked UI (X/5 card,
-- Unranked badge, "no rating yet") is gated on placement_played. So players got
-- stuck at 0/5 no matter how many rated matches they played. This re-adds the
-- placement_played increment and backfills anyone already out of step.
-- ============================================================================
...
-- Backfill anyone whose placement counter lagged behind their settled matches.
