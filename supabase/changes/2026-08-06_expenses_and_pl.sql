...
-- migrations/0003 shipped the column as `registered_at`; the canonical file
-- calls it `created_at` and its create-table block is skipped on a live
-- database, so `created_at` was never added. Add it and backfill from
-- registered_at where that exists, so historic entries keep their REAL date
-- instead of all landing in today's week. (This also repairs the entries sheet,
-- which has been ordering by a column that isn't there.)
do $$
begin
