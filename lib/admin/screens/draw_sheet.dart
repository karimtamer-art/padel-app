...
// Configurable "Draw & results" for a Groups → Knockout tournament (Pass 1).
// Setup: choose groups + advance, see the split, warnings, and outcome, then
// Generate (server builds real group matches). Draw: a live standings board —
// tap a fixture to cycle its winner; standings + qualifiers update instantly;
// Advance seeds the knockout. Single-elim/round-robin boards land in Pass 2.
...
class _DrawSheetState extends State<DrawSheet> {
  ...
  Future<void> _generate() async {
    ...
    final err = await AdminService.saveTournamentFormat(widget.tournamentId, spec) ??
        await AdminService.generateFromFormat(widget.tournamentId);
    if (!mounted) return;
    if (err != null) {
    ...
    if (!mounted) return;
    setState(() => _busy = false);
    adminToast(context, 'Group draw generated');
  }

  Future<void> _cycle(DrawFixture f) async {
    if (_busy) return;
    final next = f.winnerId == f.a.entryId
        ...
        : f.winnerId == f.b.entryId
            ...
            : f.a.entryId;
    setState(() => _busy = true);
    final err = await AdminService.setMatchWinner(f.id, next);
    if (!mounted) return;
  ...
  Future<void> _pickKo(String matchId, String? winnerEntry) async {
    if (_busy) return;
    setState(() => _busy = true);
        ...
        child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Expanded(
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Text('${widget.tournamentName} · $_pairs pairs · Groups → Knockout',
  ...
  Widget _setupBody() {
    final quals = _quals;
    final summary =
        '$_groups groups → top $_advance ($quals) → ${_roundName(nextPow2(quals))} → champion';
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      Container(
        ...
        child: Column(children: [
            ...
            child: Wrap(spacing: 6, runSpacing: 6, children: [
              for (var i = 0; i < _sizes.length; i++)
                Container(
                  ...
                  decoration: BoxDecoration(
                      color: _tone(i).withValues(alpha: 0.13),
          ...
          _cfgRow(
            'Advance per group',
            'Top $_advance qualify → $quals into the knockout',
            _Stepper(
              value: _advance,
              min: 1,
              max: (_minSize - 1).clamp(1, 99),
      ...
      Container(
        ...
        decoration: BoxDecoration(
            color: AdminColors.green.withValues(alpha: 0.09),
            borderRadius: BorderRadius.circular(11)),
        child: Row(children: [
      ...
      Container(
        ...
        decoration: BoxDecoration(
            ...
            border: Border.all(color: AdminColors.lineSoft)),
        child: Row(children: [
          const Icon(Icons.trending_up_rounded, size: 18, color: AdminColors.primary),
          const SizedBox(width: 11),
          Expanded(
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text('Seeded by level', style: AdminText.sans(13.5, FontWeight.w800, AdminColors.ink)),
              const SizedBox(height: 1),
              Text('Top pairs are kept apart across the groups.',
  ...
  Widget _lockedFormatCard() => Container(
        ...
        child: Row(children: [
          Container(
            ...
            decoration: BoxDecoration(
                color: _tone(0).withValues(alpha: 0.14), borderRadius: BorderRadius.circular(10)),
            child: Icon(Icons.groups_rounded, size: 19, color: _tone(0)),
          ),
          const SizedBox(width: 12),
          Expanded(
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text('Groups → Knockout', style: AdminText.sans(14, FontWeight.w800, AdminColors.ink)),
              const SizedBox(height: 1),
  ...
  // ── draw ─────────────────────────────────────────────────────────────────
  Widget _drawBody() => _view == 0 ? _groupsBoard() : _knockoutBoard();

  Widget _groupsBoard() {
    final groups = parseGroups(_matches);
    if (groups.isEmpty) {
      return _emptyNote('No group matches yet — regenerate the draw.');
    }
  ...
  Widget _knockoutBoard() {
    // KO matches = non-group rows, grouped by round label, biggest round first.
    final ko = _matches
        .where((m) => !((m['bracket'] as String?) ?? '').startsWith('Group '))
        .toList();
    if (ko.isEmpty) {
      return _emptyNote('Finish the groups, then Advance to seed the knockout.');
    }
    ko.sort((a, b) => ((a['round'] as num?) ?? 0).compareTo((b['round'] as num?) ?? 0));
    final byRound = <String, List<Map<String, dynamic>>>{};
    for (final m in ko) {
      byRound.putIfAbsent((m['bracket'] as String?) ?? 'Round', () => []).add(m);
    }
    return Column(children: [
      _hintBar('Tap the winning pair to advance them. Then Advance the draw for the next round.'),
      const SizedBox(height: 12),
      for (final entry in byRound.entries) ...[
        ...
        for (final m in entry.value) _koMatchCard(m),
        const SizedBox(height: 8),
      ],
    ]);
  }

  Widget _koMatchCard(Map<String, dynamic> m) {
    final a = pairOf(m['e1']);
    final b = pairOf(m['e2']);
    ...
    Widget side(DrawPair? p) {
      if (p == null) {
        return Padding(
          ...
          child: Text('Bye', style: AdminText.small(AdminColors.inkFaint)),
        );
      }
      final win = winner == p.entryId;
      final lose = winner != null && !win;
      return InkWell(
        onTap: () => _pickKo(id, win ? null : p.entryId),
        child: Container(
          ...
          child: Row(children: [
            Expanded(
              child: Text(p.lead,
                  ...
                  style: AdminText.sans(13, win ? FontWeight.w800 : FontWeight.w600,
                      lose ? AdminColors.inkFaint : AdminColors.ink)),
            ),
            if (win) const Icon(Icons.check_circle_rounded, size: 16, color: AdminColors.green),
          ]),
        ),
      );
    }
  ...
  Widget _hintBar(String text) => Container(
        padding: const EdgeInsets.all(11),
        decoration: BoxDecoration(
            color: AdminColors.info.withValues(alpha: 0.09), borderRadius: BorderRadius.circular(11)),
        child: Row(children: [
...
class _GroupCard extends StatelessWidget {
  ...
  final Color tone;
  final int advance;
  final Future<void> Function(DrawFixture) onCycle;
  ...
  Widget _hCell(String s) =>
      SizedBox(width: 28, child: Center(child: Text(s, style: AdminText.kicker())));
        ...
        SizedBox(
          ...
          child: Row(children: [
            Container(width: 3, height: 16,
                decoration: BoxDecoration(
                    color: qual ? tone : Colors.transparent, borderRadius: BorderRadius.circular(2))),
        ...
        Expanded(
          child: Row(children: [
            Container(
              ...
              decoration: BoxDecoration(
                  color: tone.withValues(alpha: 0.16), borderRadius: BorderRadius.circular(7)),
              child: Text(r.pair.initials, style: AdminText.mono(9.5, FontWeight.w800, tone)),
            ),
            const SizedBox(width: 8),
            Expanded(
              child: Text(r.pair.lead,
                  maxLines: 1, overflow: TextOverflow.ellipsis,
      ...
      child: Container(
        ...
        child: Row(mainAxisSize: MainAxisSize.min, children: [
          ...
          Padding(
            ...
            child: Text(w == null ? 'v' : '·', style: AdminText.small(AdminColors.inkFaint)),
          ),
          side(f.b),
        ]),
      ),
